...
import { JV_File, JV_Handlers, JV_InstanceType, JV_RunnerArguments } from "../types/javon";
import { JavonLoop } from './loop';
import { evaluateExpression } from '..';

export default class JavonInstance {
  public errors: LocalErrors;
  public warn: LocalWarnings;
  public loop: JavonLoop = new JavonLoop();
  private files: Map<string, JV_File> = new Map();
  
  constructor(
    public entrypoint: FileInput, 
    public type: JV_InstanceType, 
    handlers: JV_Handlers | null = null,
    public args: JV_RunnerArguments | null = null
  ) {
    if(type === "sandbox") {
